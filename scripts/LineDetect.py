...
def cam0_read():
    global frame0, original, gray0
    ret, frame0 = cap0.read()
    original = frame0
    gray0 = cv2.cvtColor(frame0, cv2.COLOR_BGR2GRAY)

def cam1_read():
    global frame1, original1, gray1
    ret, frame1 = cap1.read()
    original1 = frame1
    gray1 = cv2.cvtColor(frame1, cv2.COLOR_BGR2GRAY)

def cam1_qrcode(Qr_res):
    global Qr_data, Qr_type, qr_detected
    Qr_decoded = pyzbar.decode(gray1)
    for qr_code in Qr_decoded:
        x, y, w, h = qr_code.rect
        Qr_data = qr_code.data.decode("utf-8")
        Qr_type = qr_code.type
        cv2.rectangle(frame1, (x, y), (x + w, y + h), (0, 0, 255), 2)
        text_0 = '%s (%s)' % (Qr_data, Qr_type)
        cv2.putText(frame1, text_0, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2, cv2.LINE_AA)
...
def Start(Qr_res):
    ...
    vel_msg = Twist()
    global speed, angle, qr_detected
    while not rospy.is_shutdown():
        pub.publish(vel_msg)
        cam1_read()
        cam1_qrcode(Qr_res)
        cv2.imshow('Qr', original1)
        print(Qr_data)
        if not qr_detected:  # QR 코드를 검출하지 못한 경우에만 cam0_read()를 수행함
            cam0_read()
            cam0_line()
            cv2.imshow('Start', frame)
            cv2.moveWindow('Start', 1200, 250)
            ...
            vel_msg.linear.x = speed  # 원하는 선속도 설정
            vel_msg.angular.z = angle # 각속도는 0으로 설정
            qr_detected = False      # qr_detected 변수 재설정
            break # Qr 인식되면 종료
